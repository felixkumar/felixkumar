...
class SaleWeight(models.Model):
   ...
   do_net_weight = fields.Float(string='Net Weight',
                               store=True,
                               related='product_id.display_weight')
   do_volume = fields.Float(string='Volume',
                               store=True,
...
class SaleWeightOrder(models.Model):
    ...
    @api.depends('move_lines.do_volume')
    ...
    def _calcvolume(self):
        currentvolume = 0
        for move_lines in self.move_lines:
            currentvolume = currentvolume + (move_lines.do_volume * move_lines.do_quantity)

        self.do_volume_total = currentvolume

    do_volume_total = fields.Float(compute='_calcvolume', string='Volume')
