...
class AccountChartTemplate(models.AbstractModel):
    ...
    def _l10n_ae_corporate_tax_report_setup_account_tags(self, ae_companies):
        ...
        for company in ae_companies:
            for expense_account in self.env['account.account'].search([('company_id', '=', company.id), ('account_type', 'in', ['expense', 'expense_depreciation'])]):
                expense_account.tag_ids = [Command.link(exp_tag.id)]

            for cog_account in self.env['account.account'].search([('company_id', '=', company.id), ('account_type', '=', 'expense_direct_cost')]):
