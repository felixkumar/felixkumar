    ...
    */
    ...
    _getNewAttrsFromModifiers: function (modifiers) {
        ...
        _.each(modifiers, function (value, key) {
                ...
                if (keyInNodeAndAttrs && !isOriginNodeAttr && trueValue) { // modifier always applied, use modifier attribute
                    newAttributes[key] = "1";
                } else if (keyInNodeAndAttrs && keyFromView || !trueValue) { // modifier not applied or under certain condition, remove modifier attribute and use attrs if any
    ...
    _onDomainAttrs: function (ev) {
        ...
        dialog.on("domain_selected", this, function (e) {
            ...
            if (this.view_type === "list" && modifier === "invisible") {
                if (_.isEqual(e.data.domain, [])) {
                    delete newModifiers.invisible;
                    newModifiers["column_invisible"] = true
                } else {
                    delete newModifiers.column_invisible;
