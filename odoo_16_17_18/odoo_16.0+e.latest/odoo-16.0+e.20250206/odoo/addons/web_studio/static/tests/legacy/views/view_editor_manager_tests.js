...
QUnit.module('ViewEditorManager', {
    ...
    function getInvisibleTest({ attributes, newAttrs }) {
        return async function (assert) {
            ...
            const archReturn = `<tree>
                <field name='display_name'/>
                <field name="char_field" attrs="{'column_invisible': True}"/>
            </tree>`;
            ...
            const vem = await studioTestUtils.createViewEditorManager({
                ...
                arch: `<tree>
                    <field name="display_name"/>
                    <field name="char_field" ${attributes}/>
                </tree>`,
                mockRPC: function (route, args) {
                    if (route === '/web_studio/edit_view') {
                        assert.deepEqual(args.operations[0].new_attrs, newAttrs);
                        return getCurrentMockServer()._mockReturnView(archReturn, "coucou");
                    }
                }
            });
    ...
    QUnit.test("enabling invisible", getInvisibleTest({
        attributes: ``,
        newAttrs: {
            invisible: "1",
            attrs: "{}",
    ...
    QUnit.test("disabling invisible where column_invisible is set", getInvisibleTest({
        attributes: `attrs="{'column_invisible': True}"`,
        newAttrs: {
            attrs: "{}",
            invisible: "",
        }
    ...
    QUnit.test("disabling invisible where column_invisible and invisible are set", getInvisibleTest({
        attributes: `attrs="{'column_invisible': True}" invisible="1"`,
        newAttrs: {
            attrs: "{}",
            invisible: "",
        }
    ...
    QUnit.test("disabling invisible with dynamic invisible & column_invisible", getInvisibleTest({
        attributes: `attrs="{'invisible': [('display_name', '=', 'test')], 'column_invisible': []}"`,
        newAttrs: {
            attrs: "{}",
            invisible: "",
        }
