...
QUnit.module('Studio', {}, function () {
    ...
    QUnit.module('ReportEditorSidebar', {
        ...
        QUnit.test("colors are available on text with t-esc", async (assert) => {
            const sidebar = await studioTestUtils.createSidebar({
                state: {
                    mode: 'properties',
                    nodes: [{
                        context: {
                            someVariable: "someqwebvalue",
                        },
                        node: {
                            attrs: {
                                'data-oe-id': '99',
                                'data-oe-xpath': '/t/t/strong',
                                't-esc': "someVariable",
                            },
                            tag: 'strong',
                            $nodes: $(),
            ...
            assert.containsOnce(sidebar.$el, ".o_web_studio_colors");
            sidebar.destroy();
        })
    });
