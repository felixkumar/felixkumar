...
export default AbstractController.extend({
    ...
    _getDialogContext(date, rowId) {
        const state = this.model.get();
        const context = {};
        context[state.dateStartField] = this.model.convertToServerTime(date.clone(), state.dateStartFieldType);
        context[state.dateStopField] = this.model.convertToServerTime(
            date.clone().endOf(this.SCALES[state.scale].interval),
            state.dateStopFieldType
        );
        if (rowId) {
    ...
    _onAddClicked(ev) {
        ...
        const context = {};
        const state = this.model.get();
        context[state.dateStartField] = this.model.convertToServerTime(state.focusDate.clone().startOf(state.scale), state.dateStartFieldType);
        context[state.dateStopField] = this.model.convertToServerTime(state.focusDate.clone().endOf(state.scale), state.dateStopFieldType);
        for (const k in context) {
            context[_.str.sprintf('default_%s', k)] = context[k];
        }
        this._onCreate(context);
