...
export default AbstractModel.extend({
     ...
     *
     * @param {Moment} date
     * @param {'date'|'datetime'} [type="datetime"]
     * @returns {string} date in server format
     */
    convertToServerTime(date, type="datetime") {
    ...
    async __load(params) {
        ...
        this.ganttData = {
            ...
            dateStartFieldType: this.fields[params.dateStartField].type,
            dateStopFieldType: this.fields[params.dateStopField].type,
            groupedBy,
    ...
    rescheduleData(schedule, isUTC) {
        ...
        for (let k in data) {
            ...
            if (data[k] && (type === 'datetime' || type === 'date') && !isUTC) {
                data[k] = this.convertToServerTime(data[k], type);
            }
        };
        return data
    },
    ...
    _computeUnavailabilityRows(rows) {
        const { dateStartField, dateStopField } = this.ganttData;
        return _.map(rows, (r) => {
            if (r) {
                const records = r.records && r.records.map((record) => {
                    const res = {}
                    for (const fieldName in record) {
                        const field = this.fields[fieldName];
                        if ([dateStartField, dateStopField].includes(fieldName) && field.type === "date") {
                            res[fieldName] = this.convertToServerTime(record[fieldName], field.type);
                        } else {
                            res[fieldName] = record[fieldName];
                        }
                    }
                    return res;
                });
     ...
     */
    _getDomain() {
        const { dateStartFieldType, dateStopFieldType, startDate, stopDate } = this.ganttData;
        const domain = [
            [this.ganttData.dateStartField, '<=', this.convertToServerTime(stopDate, dateStartFieldType)],
            [this.ganttData.dateStopField, '>=', this.convertToServerTime(startDate, dateStopFieldType)],
        ];
    ...
    _parseServerData(data) {
        ...
        data.forEach((record) => {
            Object.keys(record).forEach((fieldName) => {
                const field = this.fields[fieldName];
                if ([dateStartField, dateStopField].includes(fieldName) && field.type === "date") {
                    record[fieldName] = moment(record[fieldName]);
                    return;
                }
                record[fieldName] = this._parseServerValue(field, record[fieldName]);
            });
        });
