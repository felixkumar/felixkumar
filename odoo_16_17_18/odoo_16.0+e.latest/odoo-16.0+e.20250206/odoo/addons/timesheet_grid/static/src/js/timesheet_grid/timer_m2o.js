...
const TaskWithHoursGrid = require('timesheet_grid.task_with_hours_grid');
const _t = core._t;
...
const TimerHeaderM2O = Widget.extend(StandaloneFieldManagerMixin, {
    ...
    willStart: async function () {
        ...
        this.project = await this.model.makeRecord('account.analytic.line', [{
            ...
            type: 'many2one',
            value: [this.projectId, this.projectName],
        ...
        this.createEditProjectIds = await this._rpc({
            model: 'project.project',
            method: 'get_create_edit_project_ids',
            args: [],
        });
    },
    /**
    ...
    start: async function () {
        ...
        const isProjectManager = await session.user_has_group('project.group_project_manager');
        const projectMany2one = new TimesheetFieldMany2One(this, 'project_id', projectRecord, {
            attrs: {
                string: _t('Project'),
                placeholder: placeholderProject,
                options: { no_create_edit: true, no_quick_create: !isProjectManager },
            },
            noOpen: true,
            mode: 'edit',
            required: true,
        ...
        const taskMany2one = new TaskWithHoursGrid(this, 'task_id', taskRecord, {
            attrs: {
                string: _t('Task'),
    ...
    _updateTaskOptions: function(newId){
        if (this.createEditProjectIds.includes(newId)) {
            this.taskMany2one.attrs.options.no_create_edit = false;
            this.taskMany2one.attrs.options.no_quick_create = true;
            this.taskMany2one.DisplayDiscardDialog = false;
        } else {
            this.taskMany2one.attrs.options.no_create_edit = true;
            this.taskMany2one.attrs.options.no_quick_create = false;
            this.taskMany2one.DisplayDiscardDialog = true;
        }
    }
});
