...
const CommonTimesheetGridModel = GridModel.extend(GroupByNoDateMixin).extend({
    ...
    _updateContext(records) {
        const span = this.currentRange.span != 'day' && this.currentRange.span;
        const record = records && records.length && records[0];
        if (span && record && this.currentRange.step) {
            const firstCol = record.cols[0];
            const lastCol = record.cols[record.cols.length - 1];

            const firstDate = firstCol.values.date[0].split('/')[0]
            const lastDate = lastCol.values.date[0].split('/')[0]
            const initialDate = record.initial.grid_anchor;

            if (firstDate <= initialDate && initialDate <= lastDate) {
