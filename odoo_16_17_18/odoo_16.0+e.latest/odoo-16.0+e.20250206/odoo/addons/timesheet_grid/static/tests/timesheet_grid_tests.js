odoo.define('timesheet_grid.timesheet_tests', function (require) {
    ...
    QUnit.module('Views', {
        ...
        QUnit.test('grid anchor updates when switching between months', async function (assert) {
            assert.expect(3);
            ...
            assert.strictEqual(grid.model.getContext().grid_anchor, '2024-01-31',
                'the grid anchor should be the current date');
            await testUtils.dom.click(grid.$buttons.find('button.grid_arrow_next'));
            assert.strictEqual(grid.model.getContext().grid_anchor, '2024-02-29',
                'grid anchor should move to end of next month');
            await testUtils.dom.click(grid.$buttons.find('button.grid_arrow_previous'));
            assert.strictEqual(grid.model.getContext().grid_anchor, '2024-01-31',
                'moving back, grid anchor should be current date again');
