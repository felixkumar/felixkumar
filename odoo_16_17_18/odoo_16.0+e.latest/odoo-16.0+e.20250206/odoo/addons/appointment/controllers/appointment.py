...
class AppointmentController(http.Controller):
    ...
    def appointment_form_submit(self, appointment_type_id, datetime_str, duration_str, staff_user_id, name, phone, email, **kwargs):
        ...
        if email_normalized and not Partner:
            Partner = request.env['res.partner'].sudo().search([('email_normalized', '=', email_normalized)], limit=1)
        if Partner:
            ...
            if not request.env.user._is_public() and request.env.user in Partner.user_ids:
