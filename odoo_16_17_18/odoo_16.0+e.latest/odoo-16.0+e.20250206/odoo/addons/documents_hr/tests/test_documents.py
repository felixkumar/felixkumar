...
class TestCaseDocumentsBridgeHR(TransactionCase):
    ...
    def setUp(self):
        ...
        self.employee = self.env['hr.employee'].create({
            ...
            'address_home_id': self.documents_user.partner_id.id,
        })
    ...
    def test_open_document_from_hr(self):
        ...
        self.assertEqual(context['default_partner_id'], self.employee.address_home_id.id)
        self.env['documents.document'].create([
            {'name': 'employee doc1', 'folder_id': self.folder_test.id, 'partner_id': self.employee.address_home_id.id},
        ...
        self.assertEqual(
            len(filtered_documents.filtered(
                lambda doc: doc.owner_id == self.employee.user_id or doc.partner_id == self.employee.address_home_id)),
            2,
