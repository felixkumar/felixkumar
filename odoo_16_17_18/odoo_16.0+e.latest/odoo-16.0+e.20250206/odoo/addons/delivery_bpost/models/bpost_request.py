    ...
    def _compute_boxes(self, picking, carrier):
        ...
        for package in picking.package_ids:
            ...
            boxes.append({
                ...
                'contentDescription': ' '.join(["%d %s" % (line.qty_done, re.sub(r'[\W_]+', ' ', line.product_id.name or '')) for line in package_lines])[:50],
            })
        ...
        if lines_without_package:
            ...
            boxes.append({
                'weight': str(_grams(weight_in_kg)),
                'parcelValue': max(min(int(parcel_value*100), 2500000), 100),
                'contentDescription': ' '.join(["%d %s" % (line.qty_done, re.sub(r'[\W_]+', ' ', line.product_id.name or '')) for line in lines_without_package])[:50],
            })
        return boxes
