...
class AccountMove(models.Model):
    ...
    def _perform_address_validation(self):
        ...
        # We perform address validation only after the move has a partner set, to avoid false positive
        # when the record is created with missing info (duplication of payment)
        moves = self.filtered(lambda move: move.move_type in ('out_invoice', 'out_refund'))
        return super(AccountMove, moves)._perform_address_validation() and not moves.payment_id
