...
class Task(models.Model):
    ...
    def write(self, vals):
        ...
        planned_date_end = fields.Datetime.to_datetime(vals.get('planned_date_end'))
        ...
        # If given default datetime values (00:00:00 -> 23:59:59), plan dates based on schedule
        if (
            planned_date_begin
            and planned_date_begin + timedelta(days=+1, seconds=-1) == planned_date_end
            and not self._context.get('fsm_mode')  # industry_fsm processes dates if fsm_mode=True
        ):
            # Filter tasks without existing planned dates
            compute_default_planned_dates = self.filtered(
                lambda task: not (task.planned_date_begin or task.planned_date_end)
            )
            ...
            remaining = self - compute_default_planned_dates
        else:
            remaining = self

        return super(Task, remaining).write(vals)
