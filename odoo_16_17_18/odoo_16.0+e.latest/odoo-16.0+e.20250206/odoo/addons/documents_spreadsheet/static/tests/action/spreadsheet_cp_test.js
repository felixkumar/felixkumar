...
QUnit.module(
    ...
    function () {
        ...
        QUnit.test("spreadsheet name can never be empty (white spaces)", async function (assert) {
            await createSpreadsheet();
            const input = target.querySelector(".breadcrumb-item input");
            assert.equal(input.value, "Untitled spreadsheet", "The input should have the placeholder value initially");

            await fields.editInput(input, "     ");
            await dom.triggerEvent(input, "change");
            assert.equal(input.value, "Untitled spreadsheet", "The input should retain the placeholder value when set to empty spaces");

            await fields.editInput(input, "My spreadsheet");
            await dom.triggerEvent(input, "change");
            assert.equal(input.value, "My spreadsheet", "The input should update to the new value when set to a valid name");

            await fields.editInput(input, "     ");
            await dom.triggerEvent(input, "change");
            assert.equal(input.value, "My spreadsheet", "The input should retain the new value even when set to empty spaces");
        });
