    ...
    setup() {
        ...
        onWillUpdateProps((nextProps) => {
            ...
            this.isKeepSelection =
                nextProps.selection.length === 1 ? nextProps.selection[0].isKeepSelection : false;
        });
        onPatched(() => {
            if (!this.inspectorMobileRef.el) {
                return;
            }
            if (this.shouldOpenInspector && this.isKeepSelection && !this.env.documentsView.previewStore.documentList) {
