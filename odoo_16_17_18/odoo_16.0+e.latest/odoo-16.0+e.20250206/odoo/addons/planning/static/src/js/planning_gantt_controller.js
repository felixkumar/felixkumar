     ...
     */
     _getDialogContext(date, rowId) {
         ...
         if (state.scale == "week" || state.scale == "month") {
             ...
             context[state.dateStartField] = this.model.convertToServerTime(dateStart, state.dateStartFieldType);
             context[state.dateStopField] = this.model.convertToServerTime(dateStop, state.dateStopFieldType);
         }
        if (state.scale == 'day') {
