...
class ArgentinianReportCustomHandler(models.AbstractModel):
    ...
    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals):
        ...
        if not self.env.context.get('print_mode') and len(lines) > (report.load_more_limit or 0) and self.env.ref('l10n_ar_reports.main_template_ar_vat_book', raise_if_not_found=False):
            options['ar_vat_book_skipped_lines_warning'] = True
            ...
            lines.append((0, {
                ...
                'columns': [
                    {'class': 'number', 'no_format': column_sums[col_index], 'name': report.format_value(column_sums[col_index], figure_type='monetary')}
                    if col_index in column_sums
                    else {}
                    for col_index in range(len(options['columns']))
                ],
                'level': 2,
            }))
