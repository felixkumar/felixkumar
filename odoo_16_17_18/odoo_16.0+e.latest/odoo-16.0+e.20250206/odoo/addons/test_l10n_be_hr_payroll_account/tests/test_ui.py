...
class TestUi(odoo.tests.HttpCase):
    def test_ui(self):
        ...
        self.env.ref('fleet.model_corsa').write({
            'can_be_requested': True,
        ...
        if not self.env.ref('fleet.fleet_vehicle_state_waiting_list', raise_if_not_found=False):
            waiting_list_state = self.env['fleet.vehicle.state'].create({
                'name': 'Waiting List',
                'sequence': 10,
            })
        ...
        img_path = get_module_resource('sign', 'static', 'demo', 'signature.png')
        ...
        self.env.ref('base.main_partner').email = "info@yourcompany.example.com"
