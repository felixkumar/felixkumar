...
class TestQualityCheck(TestQualityCommon):
    ...
    def test_failed_measure_quality_check(self):
        """Test that a quality check with a failed measure test is correctly validated and can be confirmed without error."""
        ...
        receipt.action_confirm()
        receipt.move_ids.quantity_done = 1
        quality_check_wizard_dict = receipt.check_quality()
        quality_check_wizard = Form(self.env[quality_check_wizard_dict['res_model']].with_context(quality_check_wizard_dict['context'])).save()

        receipt.check_ids.measure = 3.
        quality_check_wizard.current_check_id.do_measure()

        res = quality_check_wizard.confirm_measure()
