...
class TestBankRecWidget(TestBankRecWidgetCommon):
    ...
    def test_caba_reconcile_account_multi_currency(self):
        ...
        inv_line = self._create_invoice_line(
            ...
            invoice_date='2016-01-01',
            invoice_line_ids=[{'price_unit': 1200.0, 'tax_ids': caba_tax}],
        )
        ...
        self.assertRecordValues(st_line.line_ids, [
            ...
            {'balance': -690.0, 'amount_currency': -1380.0, 'currency_id': currency.id},
        ])
        ...
        self.assertRecordValues(caba_move.line_ids, [
            {'balance': 600.0,  'amount_currency': 1200.0,  'currency_id': currency.id,     'account_id': product_account.id},
            {'balance': -600.0, 'amount_currency': -1200.0, 'currency_id': currency.id,     'account_id': product_account.id},
            {'balance': 90.0,   'amount_currency': 180.0,   'currency_id': currency.id,     'account_id': cash_basis_transfer_account.id},
            {'balance': -90.0,  'amount_currency': -180.0,  'currency_id': currency.id,     'account_id': tax_account.id},
        ])
        ...
        self.assertRecordValues(exchange_diff_moves.line_ids, [
            ...
            {'balance': -30.0,  'amount_currency': 0.0, 'currency_id': currency.id},
            {'balance': 30.0,   'amount_currency': 0.0, 'currency_id': currency.id},
            # Exchange diff st_line with invoice ((1380 / 2) - (1380 / 3) = 690 - 430 = 230):
            {'balance': 230.0,  'amount_currency': 0.0, 'currency_id': currency.id},
            {'balance': -230.0, 'amount_currency': 0.0, 'currency_id': currency.id},
        ])
