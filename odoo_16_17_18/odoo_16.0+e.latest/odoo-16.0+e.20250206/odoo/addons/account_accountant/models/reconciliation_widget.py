...
class AccountReconciliation(models.AbstractModel):
    ...
    def get_data_for_manual_reconciliation(self, res_type, res_ids=None, account_type=None):
        ...
        aml_ids = self._context.get('active_ids') and self._context.get('active_model') == 'account.move.line' and tuple(self._context.get('active_ids'))
        if self._context.get('all_entries', False):
            lateral_join = """
                LEFT JOIN LATERAL (
                    SELECT 1
            ...
            """.format(inner_where=is_partner and 'AND l.partner_id = s.partner_id' or ' ')
            lateral_join_where = "residual IS NOT NULL"
        else:
            lateral_join = """
                LEFT JOIN LATERAL (
                    SELECT 1
            ...
            """.format(inner_where=is_partner and 'AND l.partner_id = s.partner_id' or ' ')
            lateral_join_where = "(residual_pos IS NOT NULL AND residual_neg IS NOT NULL)"
        ...
        """.format(
                ...
                outer_where=(is_partner and '(last_time_entries_checked IS NULL OR max_date > last_time_entries_checked) AND ' or ' ') + lateral_join_where,
    ...
    def _prepare_writeoff_moves(self, move_lines, vals):
        ...
        move_vals['line_ids'] = [
            (0, 0, line_vals),
            (0, 0, {
                'name': line_vals.get('name', _('Write-Off')),
