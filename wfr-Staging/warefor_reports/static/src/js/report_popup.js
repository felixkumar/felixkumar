...
function makeActionManager(env) {
    ...
    async function _triggerDownload(action, options, type) {
        ...
        try {
            ...
            env.services.ui.unblock();
                if ($(".o_dialog_container").hasClass("modal-open")){
                    const closeButton = document.querySelector(".o_dialog_container.modal-open .btn-close");
                    if (closeButton) {
     ...
     */
     async function _executeReportAction(action, options) {
        let freightRecords =null;
        let active_id = action.context.active_id
        const buttonElementLoaded = document.querySelector("button[name='print_report']");
        const buttonElementMaster = document.querySelector("button[name='print_report_master_bol']");
//        const buttonElementHouse = document.querySelector("button[name='print_report_house_bol']");

        if (action.report_name === 'warefor_reports.report_LoadedWithPride' && !buttonElementLoaded){
            doAction({
                type: "ir.actions.act_window",
                target: "new",
                name: "Loaded With Pride Signature",
                res_model: "loaded.pride.wizard",
                views: [[false, "form"]],
                context: {'default_freight_record': action.context.active_id},
        });
            return;
        }
        if (action.report_name === 'warefor_reports.report_MasterBillOfLading' && !buttonElementMaster){
            doAction({
                type: "ir.actions.act_window",
                target: "new",
                name: "Master BOL Signature",
...
//        if (action.report_name === 'warefor_reports.report_HouseBillOfLading' && !buttonElementHouse){
//            doAction({
//                type: "ir.actions.act_window",
//                target: "new",
//                name: "House BOL Signature",
//                res_model: "house.bol.wizard",
//                views: [[false, "form"]],
//                context: {'default_freight_record': action.context.active_id},
//        });
//            return;
...
patch(actionService, 'warefor_reports.action_service_patch', {
    start(env){
