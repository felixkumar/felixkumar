...
class ItEquipment(models.Model):
    ...
    code = fields.Char(
        tracking=True, index=True, help="Organization specific inventory code", string="Asset Tag"
    )
    brand_id = fields.Many2one("itm.equipment.brand", "Brand")
    model = fields.Char()
    # ToDo: Commented main field for change the domain and string
    ...
    partner_id = fields.Many2one(
        ...
        tracking=True,
        default=_get_partner_id,
        help="A vendor which are using this device",
    )
    ...
    warehouse_id = fields.Many2one('stock.warehouse', string="Warehouse", realted="location_id.warehouse_id", store=True)
