...
def _create_invoice_export_xsd(cr, registry):
    ...
    with open(join_path(module_path, 'data/Invoices_810.xsd'), 'rb') as xsd_file:
        contents = xsd_file.read()
        action = env['xml.xsd.import'].create({
            'name': '810 Export Invoice',
            'file': b64encode(contents),
        }).action_create_xsd()
        xsd = env['xml.xsd'].browse(action['res_id'])
        env.ref('edi_export_account.sync_document_export_invoice').xsd_id = xsd
