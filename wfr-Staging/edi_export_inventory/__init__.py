...
def _create_inventory_inquiry_export_xsd(cr, registry):
    ...
    with open(join_path(module_path, 'data/ItemRegistries.xsd'), 'rb') as xsd_file:
        contents = xsd_file.read()
        action = env['xml.xsd.import'].create({
            'name': '846 Export Inventory Inquiry',
            'file': b64encode(contents),
        }).action_create_xsd()
        xsd = env['xml.xsd'].browse(action['res_id'])
        env.ref('edi_export_inventory.sync_document_export_inventory_inquiry').xsd_id = xsd
