...
def _create_delivery_export_xsd(cr, registry):
    ...
    with open(join_path(module_path, 'data/Shipments_856.xsd'), 'rb') as xsd_file:
        contents = xsd_file.read()
        action = env['xml.xsd.import'].create({
            'name': '856 Export delivery',
            'file': b64encode(contents),
        }).action_create_xsd()
        xsd = env['xml.xsd'].browse(action['res_id'])
        env.ref('edi_export_delivery.sync_document_export_shipment').xsd_id = xsd
